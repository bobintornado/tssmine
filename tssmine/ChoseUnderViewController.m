...
@property (strong, nonatomic) IBOutlet UICollectionView *underCV;
...
- (void)viewDidLoad
{
    ...
    self.title = @"Choose Bottom";
    self.products = [[NSMutableArray alloc] init];
    //implement this if the json is huge
    //[NSThread detachNewThreadSelector:@selector(loadData) toTarget:self withObject:nil];
    
    NSString *urlString = [NSString stringWithFormat:@"%@index.php?route=%@&key=%@&part=%@",ShopDomain,@"feed/web_api/customize",RESTfulKey,@"bottom",nil];
    ...
    [NSURLConnection sendAsynchronousRequest:[[NSURLRequest alloc] initWithURL:productsURL] queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        ...
        [self.underCV performSelectorOnMainThread:@selector(reloadData) withObject:nil waitUntilDone:NO];
    }];
}
